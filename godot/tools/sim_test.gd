...
func _process(_dt: float) -> bool:
	...
	match frames:
		...
		3500:
			note("stage2_ascends", int(game.get("stage")) == 2 and not bool(game.get("victory")))
			note("lava_forged", (game.get("lavas") as Array).size() == 3)
			game.set("stage_t", 241.0)
		3600:
			note("gate_reopens_s2", not bool(game.get("gate_sealed")))
			player.global_position = Vector3(0, 0.5, -13)
		3800:
			note("tyrant_spawned", game.get("warden") != null)
			if game.get("warden") != null:
